...
🧪 Sandboxed Code Execution — Generated Python code runs in an E2B cloud sandbox (set E2B_API_KEY) with an automatic local subprocess fallback — no untrusted code runs on your machine.
...
Planner — Analyzes the user task and any prior research context. Produces a 3-step plan. Re-runs if the researcher returns poor data.

Researcher — Runs web_search (Tavily) for live web data and pdf_search against uploaded PDFs. Feeds findings to the quality check.

⚖️ Quality Check (conditional edge) — If research is thin or returns no results, loops back to Planner (max 3 iterations). Otherwise proceeds to Coder.

Coder — Has an internal agent loop: writes code → calls execute_python_code → reads errors → calls web_search for fixes → rewrites → re-executes. Repeats until the code passes or it gives up.

Executor — Runs the Coder's final output in a fresh sandbox pass. This is the ground-truth execution result that goes into the report.

⚖️ Execution Check (conditional edge) — If the executor run fails and retries remain, sends the error back to the Coder. Otherwise proceeds to Reporter.

Reporter — Compiles task, plan, research, code, and execution result (including whether it passed) into a final Markdown report with a quality score.
...
Google Generative AI (embedding-001)
...
Terminal panel with sandbox result

metrics

server → client

accuracy, processingTime, systemStatus
